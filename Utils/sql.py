...
# リップ情報取得（類似色）
SELECT_WHERE_SIMILAR_COLORCODE = "SELECT * FROM LIP_MASTER WHERE (COLORCODE >= %(downVCode)s AND COLORCODE <= %(upVCode)s) OR (COLORCODE >= %(downSCode)s AND COLORCODE <= %(upSCode)s) AND LIP_ID != %(lipId)s AND HIDDEN_FLAG != '1';"