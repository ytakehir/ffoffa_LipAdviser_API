...
# リップ情報取得（同一商品）
SELECT_WHERE_PRODUCT_ID = "SELECT * FROM LIP_MASTER WHERE PRODUCT_ID = %(productId)s AND HIDDEN_FLAG != '1';"
...
# ランキング取得（カラーコード）
SELECT_RANKING_OF_WEEK = "SELECT c.COLORCODE, COUNT(h.ID) AS COUNT FROM SEARCH_COLORCODE c JOIN SEARCH_COLORCODE_HISTORY h ON l.ID = h.SEARCH_LIP_ID_ID WHERE h.SEARCH_TIME >= DATE_SUB(CURDATE(), INTERVAL %(interval)s DAY) GROUP BY c.COLORCODE ORDER BY COUNT DESC LIMIT 10;"
...
# ランキング取得（リップ）
SELECT_RANKING_LIP = "SELECT l.LIP_ID, COUNT(h.ID) AS COUNT FROM SEARCH_LIP l JOIN SEARCH_LIP_HISTORY h ON l.ID = h.SEARCH_LIP_ID_ID WHERE h.SEARCH_TIME >= DATE_SUB(CURDATE(), INTERVAL %(interval)s DAY) GROUP BY l.LIP_ID ORDER BY COUNT DESC LIMIT 10;"

# プロダクトID（重複なし）
SELECT_DISTINCT_PRODUCT_ID = "SELECT DISTINCT PRODUCT_ID FROM LIP_MASTER WHERE HIDDEN_FLAG != '1';"
